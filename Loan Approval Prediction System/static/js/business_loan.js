/**
 * Business Loan Application JavaScript Handler
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateEMI() {
        ...
        if (loanAmount && tenure && loanAmount >= 100000 && tenure >= 1) {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        const formData = {
            ...
            business_age: parseFloat(document.getElementById('business_age').value),
            annual_turnover: parseFloat(document.getElementById('annual_turnover').value),
        ...
        if (collateral) {
            formData.collateral = collateral;
        }

        // Handle file uploads
        const itrReports = document.getElementById('itr_reports').files[0];
        if (itrReports) {
            formData.itr_reports_filename = itrReports.name;
        }

        try {
            const response = await fetch('/api/apply/business-loan', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
